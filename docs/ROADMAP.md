...
| **1.4** ✅ | Angular shell: scaffold, auth, routing, layout, generated API types | **Done.** Angular 21.2, Tailwind 4, types generated from OpenAPI. 6 unit tests and 4 Playwright E2E tests pass, covering register → skill tree → reload-stays-signed-in, cross-branch prerequisites, and sign-out. Endpoints were converted to typed result unions first — `IResult` produced **no response schemas**, which would have made type generation useless |
